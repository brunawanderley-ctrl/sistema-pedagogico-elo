...
def main():
    ...
    freq_por_aluno = {}
    MIN_AULAS_RISCO = 5  # Minimo de chamadas para considerar frequencia confiavel
    if tem_freq and 'aluno_id' in df_freq.columns and 'pct_frequencia' in df_freq.columns:
        ...
        agg_cols = {'total_aulas': ('total_aulas', 'sum')}
        if 'presencas' in df_freq.columns:
            agg_cols['presencas'] = ('presencas', 'sum')
        elif 'presentes' in df_freq.columns:
            agg_cols['presencas'] = ('presentes', 'sum')
        if 'justificadas' in df_freq.columns:
            agg_cols['justificadas'] = ('justificadas', 'sum')

        if 'total_aulas' in df_freq.columns and ('presencas' in df_freq.columns or 'presentes' in df_freq.columns):
            freq_agg = df_freq.groupby('aluno_id').agg(**agg_cols).reset_index()
            just = freq_agg['justificadas'] if 'justificadas' in freq_agg.columns else 0
            freq_agg['pct_frequencia'] = (
                (freq_agg['presencas'] + just) / freq_agg['total_aulas'].clip(lower=1) * 100
            )
            # Somente considerar alunos com chamadas suficientes
            for _, r in freq_agg.iterrows():
                if r['total_aulas'] >= MIN_AULAS_RISCO:
