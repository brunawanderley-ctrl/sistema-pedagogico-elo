...
def main():
    ...
    if df_freq.empty:
        st.warning("Dados de frequencia 2026 ainda nao disponiveis.")
        st.info(
            "A frequencia escolar sera calculada a partir da chamada diaria dos professores no SIGA. "
            "Assim que os dados forem extraidos (fato_Frequencia_Aluno.csv), esta pagina mostrara "
            "a presenca de cada aluno, alertas de risco de reprovacao por faltas (LDB 75%) e "
            "rankings por serie/unidade.\n\n"
            "**O que precisa acontecer:**\n"
            "1. Professores fazem chamada no SIGA diariamente\n"
            "2. O sistema extrai os dados automaticamente\n"
            "3. Esta pagina calcula a frequencia por aluno"
        )
        return
    ...
    if 'aluno_id' in df.columns:
        ...
        if 'presencas' in freq_geral.columns:
            just = freq_geral['justificadas'] if 'justificadas' in freq_geral.columns else 0
            freq_geral['pct_frequencia'] = ((freq_geral['presencas'] + just) / freq_geral['total_aulas'].clip(lower=1) * 100).round(1)
    else:
        freq_geral = df.copy()
    ...
    # Minimo de aulas para considerar aluno "em risco" (evita falso positivo com 1-2 chamadas)
    MIN_AULAS_RISCO = 5
    if 'pct_frequencia' in freq_geral.columns and 'total_aulas' in freq_geral.columns:
        _risco_mask = (freq_geral['pct_frequencia'] < THRESHOLD_FREQUENCIA_LDB) & (freq_geral['total_aulas'] >= MIN_AULAS_RISCO)
        em_risco = int(_risco_mask.sum())
    elif 'pct_frequencia' in freq_geral.columns:
        em_risco = len(freq_geral[freq_geral['pct_frequencia'] < THRESHOLD_FREQUENCIA_LDB])
    else:
        em_risco = 0
    ...
    with tab2:
        ...
        if 'pct_frequencia' in freq_geral.columns and 'total_aulas' in freq_geral.columns:
            risco = freq_geral[
                ...
                (freq_geral['total_aulas'] >= MIN_AULAS_RISCO)
            ].sort_values('pct_frequencia')
        elif 'pct_frequencia' in freq_geral.columns:
            risco = freq_geral[freq_geral['pct_frequencia'] < THRESHOLD_FREQUENCIA_LDB].sort_values('pct_frequencia')
        else:
            risco = pd.DataFrame()

        # Alunos excluidos por poucas aulas
        if 'pct_frequencia' in freq_geral.columns and 'total_aulas' in freq_geral.columns:
            excluidos = freq_geral[
                (freq_geral['pct_frequencia'] < THRESHOLD_FREQUENCIA_LDB) &
                (freq_geral['total_aulas'] < MIN_AULAS_RISCO)
            ]
            if not excluidos.empty:
                st.caption(
                    f"{len(excluidos)} aluno(s) com <{MIN_AULAS_RISCO} chamadas registradas "
                    f"foram omitidos (dados insuficientes para avaliar risco)."
                )
    ...
    with tab3:
        ...
        if 'aluno_id' in freq_geral.columns and 'serie' in freq_geral.columns:
            ...
            _risco_mask_serie = (freq_geral['pct_frequencia'] < THRESHOLD_FREQUENCIA_LDB)
            if 'total_aulas' in freq_geral.columns:
                _risco_mask_serie = _risco_mask_serie & (freq_geral['total_aulas'] >= MIN_AULAS_RISCO)
            risco_serie = freq_geral[_risco_mask_serie]
