...
if role == 'ceo':
    ...
    # Coordenador / Viewer
    # Rollout progressivo: 3 semanas (Doc 50 — Opção B)
    # bruna_vitoria = acesso completo desde o início
    # demais coordenadores = fase 1 (sem 5), fase 2 (sem 6), completo (sem 7+)
    from datetime import date
    _username = st.session_state.get("username", "")
    _semana = (date.today() - date(2026, 1, 26)).days // 7 + 1
    _full_access = _username == "bruna_vitoria" or _semana >= 7

    if _full_access:
        # === ACESSO COMPLETO (bruna_vitoria ou semana 7+) ===
        sections["Autogerência (diário)"] = [
            P("app_pages/home.py", "Home", "🏠"),
            P("peex_pages/15_gerador_peex_unidade.py", "Minha Semana", "🎯"),
            P("peex_pages/07_plano_acao.py", "Plano de Acao", "📝"),
        ...
        # === FASE 2 — AGIR (semana 6: 02/03) — 18 páginas ===
        sections["Autogerência (diário)"] = [
            P("app_pages/home.py", "Home", "🏠"),
            P("peex_pages/15_gerador_peex_unidade.py", "Minha Semana", "🎯"),
            P("peex_pages/07_plano_acao.py", "Plano de Acao", "📝"),
